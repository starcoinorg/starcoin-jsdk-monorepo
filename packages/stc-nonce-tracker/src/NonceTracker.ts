...
export class NonceTracker {
  ...
  async _getNetworkNextNonce(address: string): Promise<NetworkNextNonce> {
    ...
    const blockNumber: string = await new Promise((resolve, reject) => {
      return this.ethQuery.getChainInfo(
        (err: any, res: any) => {
          if (err) {
            return reject(err);
          }
    ...
    const baseCountBN: BigNumber = await new Promise((resolve, reject) => {
      return this.ethQuery.getResource(
        address,
        '0x1::Account::Account',
        (err: any, res: any) => {
          if (err) {
          ...
          return resolve(BigNumber.from(sequence_number));
        },
      );
    });
